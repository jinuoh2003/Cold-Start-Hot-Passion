...
cat <<EOF > payload_10mb.json
{ "record_info": { "bucket": "$BUCKET_NAME", "key": "dummy_10mb.txt" } }
EOF

cat <<EOF > payload_50mb.json
{ "record_info": { "bucket": "$BUCKET_NAME", "key": "dummy_50mb.txt" } }
EOF

cat <<EOF > payload_100mb.json
{ "record_info": { "bucket": "$BUCKET_NAME", "key": "dummy_100mb.txt" } }
EOF
